...
function LandingCtas() {
  return <div className="lp-actions"><Link className="lp-button lp-button--primary" href={LIVE_DESTINATION}>Launch Adaptara</Link><Link className="lp-button lp-button--secondary" href={DEMO_DESTINATION}>Try Demo</Link></div>;
}

function PublicNavigation() {
  return <header className="lp-nav"><div className="lp-container lp-nav__inner"><Brand /><nav className="lp-nav__links" aria-label="Main navigation">{LANDING_NAVIGATION.map((label) => <a key={label} href={landingAnchor(label)}>{label}</a>)}</nav><Link className="lp-nav__launch" href={LIVE_DESTINATION}>Launch App <MingcuteIcon name="arrowRight" size={15} /></Link><details className="lp-mobile-nav"><summary aria-label="Open navigation"><span /><span /></summary><nav aria-label="Mobile navigation">{LANDING_NAVIGATION.map((label) => <a key={label} href={landingAnchor(label)}>{label}</a>)}<Link href={LIVE_DESTINATION}>Launch App <MingcuteIcon name="arrowRight" size={15} /></Link></nav></details></div></header>;
}
...
function FeatureSection({ section }: { section: (typeof sections)[number] }) {
  ...
  return <section id={section.id} className={section.reverse ? "lp-feature lp-feature--reverse lp-container" : "lp-feature lp-container"}><div className="lp-feature__copy"><p className="lp-pill">{section.eyebrow}</p><h2>{section.title}</h2><p>{section.copy}</p><ul>{section.points.map((point) => <li key={point}>{point}</li>)}</ul></div>{visual}</section>;
}

export function LandingPage() {
  return <main className="lp-page">
    <PublicNavigation />
    <section className="lp-hero lp-container" aria-labelledby="landing-title"><p className="lp-status"><i aria-hidden="true" />Policy-bound portfolio intelligence on X Layer</p><h1 id="landing-title">Policy-bound intelligence<br />for your portfolio and Vaults.</h1><p>Understand what you own, delegate only what you choose, and govern each Vault with its own Financial Constitution. MARA proposes, Adaptara calculates, and your rules allow or block.</p><LandingCtas /><ProductPreview /></section>
