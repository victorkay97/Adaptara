...
const FINANCIAL_QUANTITY_OR_MULTIPLIER = /\b\d+(?:\.\d+)?\s*(?:tokens?\b|x\b)/i;
...
export function validateMaraOutput(value: unknown, context: MaraContext): MaraAnalysis {
  ...
  if (prose.some((text) => FINANCIAL_NUMBER.test(text) || FINANCIAL_QUANTITY_OR_MULTIPLIER.test(text) || CANONICAL_ASSET_QUANTITY.test(text))) throw new MaraError("invalid-model-output", "MARA produced an unsafe numeric financial claim.");
