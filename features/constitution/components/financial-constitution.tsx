...
export function FinancialConstitutionPanel({ address, client, vault, snapshot, onActiveChange, writesEnabled = false }: { address: Address; client: PublicClient; vault: VaultDiscovery; snapshot?: PortfolioSnapshot; onActiveChange?: (constitution: OnchainConstitution | null) => void; writesEnabled?: boolean }) {
  ...
  return <section aria-labelledby="constitution-title" className="constitution-panel">
    ...
    {policyReady ? <><div className="constitution-fields">{fields.map(([key, label, help]) => <label key={key}><span className="font-semibold">{label}</span><span className="mt-1 block text-xs text-[var(--muted)]">{help}</span><span className="constitution-input"><input inputMode="decimal" value={inputs[key]} onChange={(event) => change(key, event.target.value)} aria-describedby={`${key}-error`} /><b>%</b></span>{errors[key] ? <span id={`${key}-error`} role="alert" className="mt-1 block text-xs text-red-400">{errors[key]}</span> : null}</label>)}</div><TechnicalDisclosure summary="Technical policy proof"><p>{fields.map(([key,label]) => `${label}: ${draft[key].toLocaleString()} BPS`).join(" · ")}</p></TechnicalDisclosure>
      ...
      <div className="constitution-actions">{onchain.data ? <button type="button" className="button button-secondary" onClick={() => reset(onchain.data.constitution)}>Reset draft</button> : null}<ConstitutionWriteControl writesEnabled={writesEnabled} canSave={canSave} saving={saving} onSave={save} /></div></> : null}
...
export function ConstitutionWriteControl({ writesEnabled, canSave, saving, onSave }: { writesEnabled: boolean; canSave: boolean; saving: boolean; onSave: () => void }) {
  if (!writesEnabled) return <p className="disabled-explanation">Apply is unavailable in demo mode and not enabled during read-only deployment verification.</p>;
