...
import { EMPTY_CONSTITUTION } from "../constants";
...
export function FinancialConstitutionPanel({ address, client, vault, snapshot, onActiveChange }: { address: Address; client: PublicClient; vault: VaultDiscovery; snapshot?: PortfolioSnapshot; onActiveChange?: (constitution: OnchainConstitution | null) => void }) {
  ...
  const canSave = Boolean(vaultAddress && onchain.data && walletClient && owner && dirty && validation.valid && feasibility?.feasible && !Object.keys(errors).length && !saving);
  ...
  const save = async () => {
    if (!vaultAddress || !walletClient || !canSave) return;
    const startedContext = contextId;
    const startedQueryKey = queryKey;
    setSaving(true); setTx("Awaiting wallet confirmation…");
    try {
      const result = await updateVaultConstitution({ publicClient: client, walletClient, vaultAddress, connectedAddress: address, policy: draft, assets: ASSET_CATALOG });
  ...
  return <section aria-labelledby="constitution-title" className="rounded-3xl border border-white/80 bg-[var(--surface)] p-5 shadow-[0_20px_60px_rgba(34,57,43,0.08)] sm:p-7">
    ...
    {policyReady ? <><div className="mt-5 grid gap-4 md:grid-cols-2">{fields.map(([key, label, help]) => <label key={key} className="rounded-2xl border border-[var(--line)] bg-white/70 p-4"><span className="font-semibold">{label}</span><span className="mt-1 block text-xs text-[var(--muted)]">{help}</span><span className="mt-3 flex items-center gap-2"><input className="w-full rounded-xl border border-[var(--line)] px-3 py-2" inputMode="decimal" value={inputs[key]} onChange={(event) => change(key, event.target.value)} aria-describedby={`${key}-error`} /><b>%</b></span><span className="mt-2 block text-xs">Exact value: {errors[key] ? "invalid" : `${draft[key].toLocaleString()} BPS`}</span>{errors[key] ? <span id={`${key}-error`} role="alert" className="mt-1 block text-xs text-red-700">{errors[key]}</span> : null}</label>)}</div>
      ...
      <div className="mt-5 flex flex-wrap gap-3">{onchain.data ? <button type="button" className="rounded-xl border bg-white px-4 py-2 font-semibold" onClick={() => reset(onchain.data.constitution)}>Reset draft to onchain values</button> : null}<button type="button" disabled={!canSave} onClick={save} className="rounded-xl bg-[#236a4a] px-4 py-2 font-semibold text-white disabled:opacity-40">{saving ? "Confirming…" : "Save Constitution Onchain"}</button></div></> : null}
    {tx ? <p className="mt-4 text-sm" role="status" aria-live="polite">{tx}</p> : null}
    <ConstitutionComplianceSection snapshot={snapshot} policy={vaultAddress ? onchain.data?.constitution : draft} draftPreview={!vaultAddress} />
  </section>;
}

export function ConstitutionSourceBadge({ hasVault, pending, failed, loaded }: { hasVault: boolean; pending: boolean; failed: boolean; loaded: boolean }) {
  const label = !hasVault ? "Draft · not active onchain" : loaded ? "Onchain constitution" : failed ? "Onchain constitution unavailable" : pending ? "Loading onchain constitution…" : "Onchain constitution unavailable";
